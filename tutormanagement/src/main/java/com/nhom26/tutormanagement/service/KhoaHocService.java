...
import com.nhom26.tutormanagement.dto.DanhGiaResponseDTO;
...
public class KhoaHocService {
    ...
    public String duyetKhoaHoc(String idKhoaHoc, Integer trangThaiMoi) {
        ...
        if (trangThaiMoi == 1)
            return "Đã duyệt khóa học: " + khoaHoc.getTenKhoaHoc();
        if (trangThaiMoi == 2)
            return "Đã từ chối khóa học: " + khoaHoc.getTenKhoaHoc();
    ...
    public List<KhoaHocResponseDTO> getKhoaHocByGiaSu(String idGiaSu) {
        giaSuRepository.findById(idGiaSu)
    ...
    public KhoaHocResponseDTO mapToResponseDTO(KhoaHoc khoaHoc) {
        ...
        List<DanhGia> danhGias = danhGiaRepository.findByDangKyHoc_KhoaHoc_IdKhoaHoc(khoaHoc.getIdKhoaHoc());
        if (danhGias != null) {
            dto.setDanhGias(danhGias.stream().map(dg -> {
                DanhGiaResponseDTO dgDto = new DanhGiaResponseDTO();
                dgDto.setIdDanhGia(dg.getIdDanhGia());
                dgDto.setSoSao(dg.getSoSao());
                dgDto.setNoiDung(dg.getNoiDung());
                dgDto.setNgayDanhGia(dg.getNgayDanhGia());
                if (dg.getDangKyHoc() != null && dg.getDangKyHoc().getPhuHuynh() != null) {
                    dgDto.setTenPhuHuynh(dg.getDangKyHoc().getPhuHuynh().getTenPhuHuynh());
                    if (dg.getDangKyHoc().getPhuHuynh().getTaiKhoan() != null) {
                        dgDto.setAnhDaiDien(dg.getDangKyHoc().getPhuHuynh().getTaiKhoan().getAnhDaiDien());
                    }
                }
                return dgDto;
            }).collect(Collectors.toList()));
