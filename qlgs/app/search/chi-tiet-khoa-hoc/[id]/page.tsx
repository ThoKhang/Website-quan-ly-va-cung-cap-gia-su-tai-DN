...
export default function CourseDetailPage() {
  ...
  const SectionHeading = ({ icon, label }: { icon: React.ReactNode; label: string }) => (
    <div className="flex items-center gap-3 mb-5 pb-4 border-b border-slate-100">
      <div className="p-2 bg-blue-50/50 rounded-xl text-blue-600 border border-blue-100/50 shadow-sm">
        {icon}
      </div>
      <h3 className="text-xl font-black text-slate-800 tracking-tight">{label}</h3>
    </div>
  );
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] pb-24 relative">
      ...
      {/* ══════════════════════════════════
          HERO BANNER
      ══════════════════════════════════ */}
      <div className="bg-slate-900 w-full pt-6 pb-48 relative overflow-hidden">
        {/* Abstract Shapes */}
        ...
        <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-indigo-600/20 rounded-full blur-[80px] translate-y-1/3 -translate-x-1/4" />
        ...
        <div className="max-w-6xl mx-auto px-4 md:px-8 relative z-10">
          ...
          <div className="flex items-center justify-between">
            <button onClick={() => router.back()} className="flex items-center gap-2 text-white/80 hover:text-white transition-colors text-sm font-bold">
              <ChevronLeft size={18} /> Quay lại
            </button>
            <button
              onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Đã sao chép liên kết!"); }}
              className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider bg-white/10 hover:bg-white/20 text-white px-4 py-2.5 rounded-xl backdrop-blur-md transition-all border border-white/10"
            >
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 -mt-32 relative z-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-blue-900/5 overflow-hidden border border-slate-100">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-900/90 via-slate-900/40 to-transparent flex items-end p-6 md:p-8">
                  <div className="max-w-2xl">
                    <div className="flex flex-wrap gap-2 mb-4">
                      <span className="bg-blue-500 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-lg shadow-sm">
                        {course.tenMonHoc}
                      </span>
                      <span className="bg-white/20 backdrop-blur-md text-white text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-lg border border-white/20">
                        {course.tenLop}
                      </span>
                    </div>
              ...
              <div className="p-6 md:p-8 bg-white">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center justify-center text-center hover:bg-slate-100 transition-colors">
                    <Clock size={24} className="text-blue-500 mb-2" />
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Thời lượng</p>
                    <p className="text-base font-black text-slate-800">{course.soBuoiHoc || 0} buổi</p>
                  </div>
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center justify-center text-center hover:bg-slate-100 transition-colors">
                    <Star size={24} className="text-amber-500 mb-2" />
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Đánh giá</p>
                  ...
                  <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col items-center justify-center text-center hover:bg-slate-100 transition-colors">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Khu vực</p>
                    <p className="text-base font-black text-slate-800">Đà Nẵng</p>
                  </div>
                  <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-0.5">Mỗi buổi</p>
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8 space-y-8">
              <section>
                <SectionHeading icon={<Info size={20} />} label="Mô tả khóa học" />
              ...
              {course.yeuCau && (
                <section>
                  <SectionHeading icon={<ShieldCheck size={20} />} label="Yêu cầu đối với học viên" />
                  <div className="bg-amber-50/50 border border-amber-100 p-5 rounded-2xl">
                    <p className="text-sm md:text-base leading-relaxed text-amber-900 font-medium">
                      {course.yeuCau}
                    </p>
                  </div>
                </section>
              ...
              {course.noiDungKhoaHoc && (
                <section>
                  <SectionHeading icon={<ClipboardList size={20} />} label="Nội dung giảng dạy" />
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-black text-slate-800 flex items-center gap-3">
                  <div className="p-2 bg-yellow-50 rounded-xl text-yellow-500"><MessageSquare size={20} /></div>
                  Phản hồi từ Phụ huynh
                </h3>
                <div className="text-[11px] font-black text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                {(course as any).danhGias && (course as any).danhGias.length > 0 ? (
                  (course as any).danhGias.map((dg: any) => (
                    <div key={dg.idDanhGia} className="flex flex-col gap-4 p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-white hover:shadow-md transition-all">
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className="h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center text-slate-700 font-black shadow-inner border-2 border-white">
                            {dg.anhDaiDien ? <img src={dg.anhDaiDien} alt="avt" className="h-full w-full object-cover rounded-full" /> : dg.tenPhuHuynh?.charAt(0) || "P"}
                          </div>
                          <div>
                            <Text className="text-sm font-bold text-slate-800">{dg.tenPhuHuynh || "Ẩn danh"}</Text>
                            <Text className="text-[10px] text-slate-400 font-medium">{new Date(dg.ngayDanhGia).toLocaleDateString("vi-VN")}</Text>
                          </div>
                        </div>
                        <div className="bg-white px-2 py-1 rounded shadow-sm border border-slate-100 flex gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} size={12} className={i < dg.soSao ? "fill-amber-400 text-amber-400" : "fill-slate-100 text-slate-200"} />
                          ))}
                        </div>
                      </div>
                      <p className="text-sm text-slate-600 leading-relaxed italic line-clamp-3">"{dg.noiDung}"</p>
                    </div>
                  ))
                ) : (
                  <div className="col-span-full py-12 text-center bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
                    ...
                    <p className="text-sm font-medium text-slate-500">Khóa học này mới mở, chưa có đánh giá nào.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="sticky top-6 space-y-6">
              ...
              <div className="bg-white rounded-[2rem] shadow-xl shadow-blue-900/5 border border-slate-100 p-6 md:p-8">
                <div className="mb-6 text-center">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Học phí trọn khóa</span>
                  <p className="text-4xl font-black text-blue-600 mb-1">{formatCurrency(course.soTienHoc)}</p>
                  <p className="text-xs text-slate-500 font-medium">Cam kết không phát sinh phụ phí</p>
                </div>
              ...
              {tutor && (
                <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6">
                  <div className="flex items-center justify-between mb-5">
                    <h4 className="text-sm font-black text-slate-800 uppercase tracking-wider">Thông tin Gia sư</h4>
                  </div>
                  ...
                  <div className="flex gap-4 items-center mb-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    ...
                    <div>
                      <h4 className="text-lg font-black text-slate-900 leading-tight mb-1">{tutor.tenGiaSu}</h4>
                      <div className="flex items-center gap-1">
                        <Star size={12} className="fill-amber-500 text-amber-500" />
                        <span className="text-xs font-bold text-slate-700">{tutor.soSaoTrungBinh?.toFixed(1) || "5.0"}</span>
                        <span className="text-[10px] font-medium text-slate-400">({tutor.soLuongDanhGia || 0} đánh giá)</span>
                      </div>
                    </div>
                  </div>

                  <Link href={`/search/gia-su/${tutor.idGiaSu}`} className="block w-full">
                    <button className="w-full py-3 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors border border-blue-100 flex items-center justify-center gap-1.5">
                      Xem hồ sơ năng lực <ChevronRight size={14} />
                    </button>
                  </Link>
                  ...
                  {tutor.khoaHocs && tutor.khoaHocs.filter(kh => kh.idKhoaHoc !== idKhoaHoc).length > 0 && (
                    <div className="mt-6 pt-6 border-t border-slate-100">
                      <p className="text-xs font-black text-slate-800 mb-4 flex items-center gap-2">
                        <PlayCircle size={16} className="text-blue-500"/> Các khóa học khác
                      </p>
                      <div className="grid gap-3">
                        {tutor.khoaHocs.filter(kh => kh.idKhoaHoc !== idKhoaHoc).slice(0, 3).map((kh) => (
                          <Link key={kh.idKhoaHoc} href={`/search/chi-tiet-khoa-hoc/${kh.idKhoaHoc}`}>
                            <div className="group p-3 rounded-xl bg-slate-50 hover:bg-white border border-slate-100 hover:border-blue-200 hover:shadow-md transition-all cursor-pointer">
                              <h5 className="text-xs font-bold text-slate-700 group-hover:text-blue-600 line-clamp-1 mb-2 transition-colors">
                                {kh.tenKhoaHoc}
                              </h5>
                              <div className="flex justify-between items-end">
                                <div>
                                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5">{kh.tenMonHoc}</span>
                                  <span className="text-[10px] font-medium text-slate-500">{kh.tenLop}</span>
                                </div>
                                <span className="text-sm font-black text-blue-600">{formatCurrency(kh.soTienHoc).split(' ')[0]}K</span>
                              </div>
                            </div>
                          </Link>
                        ))}
                      ...
                      {tutor.khoaHocs.length > 4 && (
                        <Link href={`/search/gia-su/${tutor.idGiaSu}`} className="block text-center mt-3">
                          <span className="text-[10px] font-bold text-slate-400 hover:text-blue-600 uppercase tracking-widest transition-colors">
